...
{
	mm_handler.last_alloc = (uint8_t*)base_addr;
	mm_handler.heap_start = (uint8_t*)base_addr;
	mm_handler.heap_end = (uint8_t*)0x28000;
...
uint8_t *_create_block(uint32_t size)
{
	...
	mm_handler.last_alloc = (uint8_t*)_next_block(b_ptr);
...
char* realloc(void *mem, size_t size)
{
	...
	if((uint32_t)mem == (uint32_t)mm_handler.last_alloc || block->size <= size){
		block->size = size;
		return mem;
