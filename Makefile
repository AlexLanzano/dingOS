...
# Debug Targets
run:
	qemu-system-x86_64 -hda dingos-x86_64.img -d cpu_reset -nographic

run-debug:
	qemu-system-x86_64 -s -S -hda dingos-x86_64.img -d in_asm,guest_errors,cpu_reset -nographic

run-debug-boot:
	qemu-system-i386 -s -S -hda dingos-x86_64.img -d in_asm,guest_errors -nographic

gdb:
	gdb -ex "file kernel/x86_64/boot.elf" -ex "set arch i386:x86-64" -ex "target remote localhost:1234"
